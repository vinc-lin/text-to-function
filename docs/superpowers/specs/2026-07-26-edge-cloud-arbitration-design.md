...
│
└─ [5] 端侧仲裁器
        先看候选组合，再看采纳内容是否含不可撤销动作
          含动作 =「开口前定稿」裁决完成前不执行、不播报结论
          不含   =「先开口后补」端先占住时间轴，云回来再定
        │
        ├─ 端置信低
        │     ├─ 含动作 ──→ REJECT   无执行许可，覆盖以下所有组合
        │     └─ 不含 ────→ 不适用   回话不需要执行许可
        │
        ├─ 端+云 一致
        │     └─ 含动作 ──→ EXECUTE + SPEAK   判断取端 · 参数取云
        │
        ├─ 端+云 冲突                                    ← 核心对比
        │     ├─ 含动作 ──→ CLARIFY   两个都不执行
        │     └─ 不含 ────→ AMEND     改口，必须显式
...
图里空缺的分支不是遗漏：`一致 / 冲突` 是动作之间的关系，`互补 / 等价 / 冲突` 是内容之间的关系，两者只在 `冲突` 上重合——**而那恰好是裁决相反的那一格**。
...
flowchart TD
  ...
  EDGE -- "端候选 + 端置信" --> ASK{"要不要上云？<br/>能出车吗 · 网络可用吗<br/>端是否已足够确定"}
  ASK -- "否" --> VOID["云候选为空"]
  ASK -- "是" --> CLOUD["云端模型"]
  ...
  G0 -- "含动作" --> R0[["REJECT<br/>无执行许可 · 覆盖以下所有"]]
  G1 -- "含动作" --> R1[["EXECUTE + SPEAK<br/>判断取端 · 参数取云"]]
  G2 -- "含动作" --> R2[["CLARIFY<br/>两个都不执行"]]
  G2 -- "不含" --> R3[["AMEND 改口<br/>必须显式"]]
  G3 -- "不含" --> R4[["SPEAK → AMEND 追加"]]
  G4 -- "不含" --> R5[["丢弃云候选"]]
  G5 -- "含动作" --> R6[["EXECUTE"]]
  G5 -- "不含" --> R7[["SPEAK 端的答案"]]
  G6 -- "含动作" --> R8[["CLARIFY<br/>不执行"]]
  G6 -- "不含" --> R9[["SPEAK 云的内容"]]
  G7 -- "含动作" --> R10[["REJECT · 并说明"]]
  G7 -- "不含" --> R11[["SPEAK 诚实兜底<br/>绝不编造"]]
```
